# creating a server for Peer to Peer network
import socket
import threading
import time
import pickle
...
class Server:
    ...
    periodic_conn_test_thread: threading.Thread
    ...
    def endSystem(self):
        ...
        del self.accept_thread
        del self.periodic_conn_test_thread
    ...
    def send_fileDetail(self, c: socket.socket,fname, owner):
        file_detail = self.df.where((self.df['fname'] == fname) & (self.df['owner']==owner))
        # Iterate over each row 
        file = file_detail.head(1)
        data = pickle.dumps(file.values.tolist()[0])
        c.send(data)
        ...
        while True:
            ...
            try:
                ...
                if msg == 'get_files':
                    ...
                    # Iterate over each row 
                    my_list =[]
                    c.send(str(file_detail.shape[0]).encode(FORMAT)) 
                ...
                if msg.split(' ')[0] == 'fetch':
                    fname = msg.split(' ')[1]
                    owner = msg.split(' ')[2]
                    fileInfo_thread = threading.Thread(target = self.send_fileDetail, args =(c, fname,owner, ))
